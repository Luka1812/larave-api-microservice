...
class AMQPService
{
    ...
    public function setExchangeType(string $exchangeType) : void
    {
        if (in_array($exchangeType, $this->exchangeTypes)) {
            ...
            throw new AMQPException(
                AMQPErrorCode::getDescription(AMQPErrorCode::ERR_WRONG_EXCHANGE_TYPE),
                AMQPErrorCode::ERR_WRONG_EXCHANGE_TYPE
            );
        }
    }
    ...
    private function init() : void
    {
        try {
            $this->connection = new AMQPStreamConnection(
                ...
                (int) config('amqp.connection.port'),
                config('amqp.connection.user'),
                config('amqp.connection.pass'),
                config('amqp.connection.vhosts')
            );
            ...
            if (! $this->queueName) {
                throw new AMQPException(
                    AMQPErrorCode::getDescription(AMQPErrorCode::ERR_MISSING_QUEUE_NAME),
                    AMQPErrorCode::ERR_MISSING_QUEUE_NAME
                );
            }
            ...
            throw new AMQPException(
                AMQPErrorCode::getDescription(AMQPErrorCode::ERR_INIT_FAILED),
                AMQPErrorCode::ERR_INIT_FAILED
            );
        }
    }
     ...
     * @param array $payload
     * @param int $priority (1-10, 1 is the highest priority)
     * @return void
     *
     * @throws AMQPException
     */
    public function send(AMQPSendMessageInterface $service, array $payload, int $priority = 1) : void
    {
        if (! $this->isOpenConnection()) {
            throw new AMQPException(
                AMQPErrorCode::getDescription(AMQPErrorCode::ERR_INIT_FAILED),
        ...
        if (empty($payload)) {
            throw new AMQPException(
                AMQPErrorCode::getDescription(AMQPErrorCode::ERR_EMPTY_PAYLOAD),
                AMQPErrorCode::ERR_EMPTY_PAYLOAD
            );
        }
     ...
     * @return void
     *
     * @throws \App\Exceptions\AMQPException
